...
async function executeSearch(event) {
    ...
    function selectStudent(data) {
        ...
        // Store selected ID so submitUnifiedSitIn uses correct value
    ...
    async function timeOut(idNumber, sitInId) {
        ...
        try {
            const res = await fetch(`${BASE_URL}/time-out/${sitInId}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' }
            });
            if (res.ok) {
    ...
    function renderTable() {
        ...
        tbody.innerHTML = pageData.map(r => {
            ...
            <tr>
                ...
                <td>${isActive
                    ? `<button class="btn-timeout" onclick="timeOut('${r.idNumber}','${r.id}')"><i class="fa fa-sign-out"></i> Time Out</button>`
    ...
    const sortKeys = ['id', 'idNumber', 'lastName', 'purpose', 'lab', 'remainingSession', 'timeOut'];
